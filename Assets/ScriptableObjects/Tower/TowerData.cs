...
public class TowerData : ScriptableObject
{
	...
	// Runtime данные
	[HideInInspector] public int currentLevel = 1; // Поточний рівень башки (1-5)
	...
	public void ApplyUpgradeLevelStats()
	{
		if (!CanUpgrade())
		{
			Debug.LogWarning($"❌ {name} вже на максимальному рівні {currentLevel}!");
			return;
		}
		...
		if (nextLevel != null)
		{
			...
			Debug.Log($"✅ {name} апгрейдена до рівня {currentLevel}! " +
					  $"Урон: {damage:F1}, Радіус: {range:F1}, Швидкість: {(1f / shootInterval):F2}");
		}
	}
}
